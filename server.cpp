...
t_rcv_data rcv_data, data_collection[10];
...
int main(int argc, char *argv[]){
	char ack_msg[50] = "Server received data"; 
	if (argc==1){
	...
	int tid=0;
	uint8_t num_pack_rcv = 0;			// From 0 to 9
	/* Create a thread to delete the main thread */
...
//    printf("\033[2J");      	  // Erase the screen
	...
	while(!init_comunication){
		
		if (recvfrom(sock_fd, messageBuffer, sizeof(messageBuffer), 0, reinterpret_cast<struct sockaddr*>(&client), (socklen_t*) &len)==-1) perror("recvfrom:");
		
		memcpy(reinterpret_cast<char*>(&rcv_data), messageBuffer, sizeof(rcv_data));
		
		if(std::string(messageBuffer) == std::string("Hello Server")) init_comunication = 1;
		else{ 
	...
	#ifdef DEBUG
	cout << "[DEBUG] Inicio de la comunicacion" << endl;
	#endif
	sprintf(messageBuffer, "Hello RPI");
	...
	*/
	while (true){
		...
		if (recvfrom(sock_fd, messageBuffer, sizeof(messageBuffer), 0, reinterpret_cast<struct sockaddr*>(&client), (socklen_t*) &len)==-1) perror("recvfrom:");
		num_pack_rcv += num_pack_rcv == 10 ? -9 : 1;
		cout<<".";
		// Deserialize data
		memcpy(reinterpret_cast<t_rcv_data*>(&rcv_data), messageBuffer, sizeof(rcv_data));

		usleep(5000);
		if (sendto(sock_fd, (void*) ack_msg, (size_t) strlen(ack_msg)+1, 0, (sockaddr*) &client, (socklen_t) len)==-1) perror("sendto: ");
    
		// print_raw_data(rcv_data);
		data_collection[num_pack_rcv] = rcv_data;
		if(num_pack_rcv == 9){
			cout<<"Data Received from client ("<< inet_ntoa(client.sin_addr) << "):  " << messageBuffer << endl << endl;
			calc_stadistics(data_collection);
			print_stadistics(data_mean, data_max, data_min, data_deviation, rcv_data);
		}
		fflush(stdout);
	}//end of while loop
